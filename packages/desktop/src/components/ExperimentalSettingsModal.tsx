  ...
  function handleSettingsChange(event) {
    if (event.target.name == "face_tracking") {
      updateExperimentalSettings(
        "faceTracking",
        faceTrackingEnabled ? false : true
      );
    ...
    >
      <Modal.Header>
        <Modal.Title className="font-weight-bolder">
          <FontAwesomeIcon icon={faFlask} className="mr-2" /> Experimental
          Features
        </Modal.Title>
        <Button
      ...
      <Modal.Body>
        <p className="text-bold pt-0 mt-0">
          Warning: Experimental features may be incomplete, unreliable, and
        ...
        <p className="text-muted pt-1">
          Face tracking uses an on-device neural network to crop your camera's
          view to only show your face and a small area around it.
        </p>
        <p className="text-muted small">
          Enabling this feature may cause your video to skip at times and has
          not been tested on lower-end computers.
        </p>
      </Modal.Body>
    </Modal>
  );
